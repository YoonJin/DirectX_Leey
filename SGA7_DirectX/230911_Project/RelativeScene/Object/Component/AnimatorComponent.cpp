    ...
    : IComponent(context, actor, transform)
{
    ...
    ai_script_component = actor->GetComponent<AIScriptComponent>();
}
...
void AnimatorComponent::Update()
{
    ...
    if (this->GetAnimationsName() == "PlayerMove")
    {
        if (frame_counter > GetCurrentKeyframe(move_script_component->GetCurrentDirection())->time)
        {
            ...
            case RepeatType::Loop:
                current_frame_number %= (current_animation.lock()->GetKeyframes(move_script_component.get()->_curDir).size());
                break;
            }
            frame_counter = 0.0f;
        }
    ...
    else if (this->GetAnimationsName() == "EnemyMove")
    {
        if (frame_counter > GetCurrentKeyframe(ai_script_component->GetCurrentDirection())->time)
        {
            current_frame_number++;
            ...
            case RepeatType::Once:
                if (current_frame_number >= current_animation.lock()->GetKeyframeCount())
                {
                    current_frame_number = current_animation.lock()->GetKeyframes(ai_script_component.get()->_curDir).size() - 1;
                    Pause();
                }
                break;
            case RepeatType::Loop:
                current_frame_number %= (current_animation.lock()->GetKeyframes(ai_script_component.get()->_curDir).size());
                break;
            }
            frame_counter = 0.0f;
        }
