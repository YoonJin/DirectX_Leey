	...
	*/
	...
	char* XMLNode::ParseDeep(char* p, StrPair* parentEndTag, int* curLineNumPtr)
	{
		...
		while (p && *p) {
			...
			if (ele) {
				...
				if (mismatch) {
					_document->SetError(XML_ERROR_MISMATCHED_ELEMENT, initialLineNum, "XMLElement resourceName=%s", ele->Name());
					DeleteNode(node);
					break;
				}
			}
	...
	char* XMLElement::ParseAttributes(char* p, int* curLineNumPtr)
	{
		...
		while (p) {
			p = XMLUtil::SkipWhiteSpace(p, curLineNumPtr);
			if (!(*p)) {
				_document->SetError(XML_ERROR_PARSING_ELEMENT, _parseLineNum, "XMLElement resourceName=%s", Name());
				return 0;
			}
			...
			if (XMLUtil::IsNameStartChar(*p)) {
				...
				if (!p || Attribute(attrib->Name())) {
					...
					_document->SetError(XML_ERROR_PARSING_ATTRIBUTE, attrLineNum, "XMLElement resourceName=%s", Name());
					return 0;
				}
